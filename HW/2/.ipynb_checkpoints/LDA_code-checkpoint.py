...
class LDA:
    ...
    def compute_likelihoods(self, x):
        ...
        idx = 0
        for name in self.class_names:
            likelihoods[idx] = multivariate_gaussian_pdf(x, self.means[name], self.cov) # equal var-cov
            idx = idx + 1
    ...
    def compute_probabilities(self, x, priors):
        ...
        no_likelihood = self.likelihoods[0]
        ...
        self.posterior = {'no': no_posterior, 'yes': yes_posterior}
    ...
    def decision(self):
        if self.posterior['no'] > self.posterior['yes']:
            print("Decision: no")
        else:
            print("Decision: yes")
        print()
        
    def cost_decision(self, cost):
        print("LDA Costs: ")
